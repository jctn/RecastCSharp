    ...
    public class Graph
    {
        ...
        public Graph(int w, int h, int maxLevel, int clusterSize)
        {
            ...
            for (int i = 0; i < w; ++i)
            {
                for (int j = 0; j < h; ++j)
                {
                    var gridTile = new GridTile(i, j);
                    nodes.Add(gridTile, new Node(gridTile));
                }
            }
            ...
            foreach (Node n in nodes.Values)
            {
                ...
                for (int i = -1; i < 2; i += 2)
                {
                    SearchMapEdge(nodes, n, n.pos.x + i, n.pos.y, false);

                    SearchMapEdge(nodes, n, n.pos.x, n.pos.y + i, false);
                }
                ...
                for (int i = -1; i < 2; i += 2)
                {
                    for (int j = -1; j < 2; j += 2)
                    {
                        SearchMapEdge(nodes, n, n.pos.x + i, n.pos.y + j, true);
                    }
                }
            }
            ...
            for (int i = 0; i < maxLevel; ++i)
            {
                if (i != 0)
                    ...
                    clusterSize *= 3; //Scaling factor 3 is arbitrary
        ...
        /// </summary>
        private void SearchMapEdge(Dictionary<GridTile, Node> mapNodes, Node n, int x, int y, bool diagonal)
        {
        ...
        private bool IsInRange(GridTile tile)
        {
        ...
        private void CreateConcreteBorderNodes(Cluster c1, Cluster c2, bool x)
        {
            ...
            for (i = iMin; i < iMax; ++i)
            {
                if (x && (nodes.ContainsKey(new GridTile(c1.Boundaries.Max.x, i)) &&
                          nodes.ContainsKey(new GridTile(c2.Boundaries.Min.x, i)))
                    || !x && (nodes.ContainsKey(new GridTile(i, c1.Boundaries.Max.y)) &&
                              nodes.ContainsKey(new GridTile(i, c2.Boundaries.Min.y))))
                {
                    lineSize++;
                }
                else
